...
Deep answers flow by leaf-addressing, with one durable binding per surfaced question. `ask` with a `child_id` is contextual like the plain ask: at the root it surfaces to the user and records the pending-ask row; at a child it re-raises the question to that child's parent by authoring the Ask event, and the re-raising session waits with its own Ask block at the end of its thread. Every authored Ask event carries the ORIGIN leaf — the session that asked its own question at the bottom of the raise — stamped mechanically by the authoring loop: a session asking its own question stamps itself, a session re-raising stamps the origin leaf its child's Ask event carried. The model never supplies it. Each level validates the `child_id` it names is its own direct child and is waiting on a question; the parent binds the surfaced question to the origin its child's Ask event preserves, so the binding is right even when a raise chain interleaves with other questions. The surfaced Ask block names the direct child the raiser surfaced, and the pending-ask row names that same direct child and carries the origin leaf: the row means the same thing at every level — the one-pending gate, a `message_child` cancel, and the raised child's next event all key off the direct child the raiser can actually message, while the leaf stays internal to the binding, out of every model's view. A user redirect therefore cannot deadlock the root into silence: the root model cancels by messaging the direct child its surfaced ask names, which clears the row at once, and the cancellation resolves down the chain to the leaf. The user's answer routes by the existing mechanical path straight to the origin leaf's own session row and loop (the store and the loop registry address any session by id), so no intermediate model ever paraphrases or relays it. The root's model only ever names its own child (the session it saw in the Ask event); it never needs to know the leaf. One question is raised at a time at every level: a session that re-raised records the child it raised in its own pending-ask row, refuses a further `ask` while the row stands (a clear tool error naming the raised child), and clears the row when the raised child authors its next event — which proves its question closed, since a waiting child authors nothing — or when the session messages that child to cancel it. Intermediate levels that re-raised stay waiting and are woken by the leaf's completion reports climbing the tree. `todowrite` is root-only twice over: children never see it in the schema, and a call from a child is refused. The persona catalog (name + description, never prompt text) composes into the system prompt only for sessions whose advertised surface includes `spawn`.
- Note: `message_child` to a supervising child yields no direct report until its subtree resolves — the child is waiting on its own children, so the message queues and the reply comes with the child's completion, once nothing it spawned can still act.
- Note: `bosun list` and the panes drop rows whose parent row is gone; the S8 stop cascade owns removing a stopped subtree's rows.
